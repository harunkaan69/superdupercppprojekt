...
bool world::existsTileAt(int pX, int pY) const
{
...
{
	auto it = std::find(creatureList.begin(), creatureList.end(), creature);
...
void world::placeCreature(const std::shared_ptr<creature>& pCreature, int pX, int pY)
{
	...
	tiles[pY * width + pX].creatureList.push_back(pCreature);
}
